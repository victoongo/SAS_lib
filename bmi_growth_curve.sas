...
data merged_var (keep=nestid mom_nestid BMI_LMP_kgm2 mat_gest_wt_gain_category2 mat_ht_m
					  gest_weight_gain_kg parity_3cat mom_age_delv race_final education_final GestAge_TotalDays 
					  GEST_DIABETES PREGNANCY_HYPERTENSION baby_weight baby_gender
					  maternal_smoking maternal_smoking2 active_smoke_preg passive_smoke_preg active_passive_smoke_preg);
	set "P:\NEST I, II, and SR Harmonized Vars\nest_i_ii_sr_merge.sas7bdat"
	...
	*else if education_final in (4 5) then education4=3; /*College Graduate */
	...
	if education_final = "High school grad/GED" then education_final = "HS GED Some college"; /* 1=LT High School */
	else if education_final = "Some college" then education_final = "HS GED Some college"; /*2 = High School or GED or Some College */
...
data "D:\Dropbox\Projects\baby_growth\data\bmi";
	set _cdcdata;
run;
proc export data=_cdcdata outfile="D:\Dropbox\Projects\baby_growth\data\bmi.csv" dbms=csv replace; run;

*proc hlm data=vital_all;
*run;

/*
proc contents data="P:\NEST I, II, and SR Harmonized Vars\nest_i_ii_sr_merge.sas7bdat";run;

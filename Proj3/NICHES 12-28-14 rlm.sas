libname nest "M:\NEST I, II, and SR Harmonized Vars";

PROC IMPORT OUT= WORK.METHYL1 
            DATAFILE= "M:\NICHES\SOURCE_DATA\SLC6A2r.txt" 
            DBMS=TAB REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
run;
...
PROC IMPORT OUT= WORK.METHYL2 
            DATAFILE= "M:\NICHES\SOURCE_DATA\MMP9r.txt" 
            DBMS=TAB REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
run;
...
PROC IMPORT OUT= WORK.METHYL3 
            DATAFILE= "M:\NICHES\SOURCE_DATA\MEG31Gr.txt" 
            DBMS=TAB REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
run;
...
PROC IMPORT OUT= WORK.METHYL4 
            DATAFILE= "M:\NICHES\SOURCE_DATA\MEG3CBS1r.txt" 
            DBMS=TAB REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
run;
...
DATA NEW;
SET nest.Nest_r01_r21_blq_med_27dec14;
run; 
...
/*MT - THERE ARE DUPLICATES FOR SOME OF THE IDS on the methylation*/
/*see aduptest dataset output below*/
/*need to decide what to do with duplicates prior to analysis*/
proc sort data=adjustvars out=test dupout=aduptest nodupkey;
by nestid;
run;


/*MT - NEED TO Categorize raw variables prior to running freq*/
/*see categories from paper*/
...
data ls_m_;
  merge ls_m(keep=effect estimate &pred0 rename = (estimate = mean))
        ls_m_d(keep=effect &pred0. _&pred0. estimate stderr lower upper probz);
  by effect &pred0;
...
/*MT - most of this code isn't going to work properly since the variables you are using */
/*from the adjustvars dataset don't match the variables below*/
/*for example BABY_GENDER in adjustvars has values of 1 and 2 */
/*whereas below you are trying to use values of "1_MALE  " and "2_FEMALE"*/

...
*******  macros for calculations ******;
	...
	%macro _cuts(var,out,l,u);
		if &L <= &var <= &u then &out=0; else if &var> &u then &out=1; else if .< &var < &L then &out= -1; %mend _cuts;
************ End of Macros ************;

data _orig _mydata _old; set mydata; _id=_n_; 
	if agemos gt 240 then output _old; 	output _orig;

	if 0 <= agemos <24 then do; length=height; height=.; end; 
...
data crefage; set refdir.CDCref_d; where denom='age'; *contains all merged LMS data - use 'denom' variable;
...
data creflg; set refdir.CDCref_d (keep=denom sex _lg1--_swlg2);  where denom='length'; _htcat=_lg1;
...
data crefht; set refdir.CDCref_d (keep=denom sex _ht1 _ht2 _lwht1 _lwht2 _mwht1 _mwht2 _swht1 _swht2); where denom='height'; 
	_htcat=_ht1;  proc sort data=crefht; by sex _htcat; 
...
data _outdata; merge finfage lenht; by _id; 
...
data _outdata; set _outdata _old; ** combine with older, excluded people;
	...
	label waz='weight-for-age Z'  wapct='weight-for-age percentile'  bmiz='BMI-for-age Z'  bmipct='BMI-for-age percentile' 
		...
		_fwaz='modified weight-for-height Z'  _bivwt='BIV weight-for-age'  headcz='head_circ-for-age Z' 
		headcpct='head_circ-for age perc' 	_Fheadcz='modified head_circ Z'  _bivhc='BIV head_circ'  
		_fbmiz='modified BMI-for-age Z'  _bivbmi='BIV BMI-for-age' whpct='weight-for-height percentile'
		bmipct95='% of 95th BMI percentile'  _bivht='BIV height-for-age'  _fhaz='modified height-for-age Z' 
		_bivwh='BIV weight-for-height'  _fwhz='modified weight-for-height Z'  _bivlow='any low BIV'  _bivhigh='any high BIV'; 

proc sort data= _outdata; by _id; proc sort data=_orig; by _id;

data _cdcdata; update _outdata _orig; by _id;  *have variables in _orig dataset overwrite any changes that were made;
